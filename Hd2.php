...
<link rel ="stylesheet" href = "Hd2_style1.css" type = "text/css" />
<script type = "text/javascript" src = "Hd2_java.js"></script>
...
function readTextFile(){
    ...
    tinyMCE.get('display').setContent(xhttp.responseText);
  	...
  	xhttp.open("GET", "tinyMCE_Data/Hd2.html", true);
  	xhttp.send();
...
<?php
    ...
    //get the header text
    $sql = "SELECT text FROM hd2 where name = 'header'";
    ...
    $sql = "SELECT text FROM hd2 where name = 'subheader'";
    $result = mysqli_query($conn, $sql);
...
<form>
        ...
        <label id = "nav1"><?php echo $nav1T ?></label>&nbsp&nbsp
        <label id = "nav2"><?php echo $nav2T ?></label>&nbsp&nbsp
        <label id = "nav3"><?php echo $nav3T ?></label>&nbsp&nbsp
        <label id = "nav4" ><a href = "Hd1.php" style="color:#163576;"><?php echo $nav4T ?></a></label>&nbsp&nbsp
        <input type = "text" id = "nav5" value="<?php echo $nav5T ?>" size = "20"/>&nbsp&nbsp
		...
		while(($file=readdir($opendir))!== FALSE)
			{
				...
				if ($file!="."&&$file!="..")
				{
					...
					</div>"."<a style='color:#163576;'href='Hd2Photo.php'>Change Profile Photo</a>
...
<div><img id="uploadimage"  src="setHd2Photo/image.jpg"  alt="uploadImage"></div>
<a href="Hd2Photo.php" style="color:#163576;">Change profile photo</a><br>
...
<button><a href = "Hd2Update.php">Publish the website</a></button><br><br>
